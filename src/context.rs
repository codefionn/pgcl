    ...
    pub fn set_values_in_context(
        ...
        match (lhs, rhs) {
            (Syntax::Tuple(a0, b0), Syntax::Tuple(a1, b1)) => {
                let a = self.set_values_in_context(a0, a1, values_defined_here);
                let b = self.set_values_in_context(b0, b1, values_defined_here);
                a && b
            }
            ...
            (Syntax::Lst(lst0), Syntax::Lst(lst1)) if lst0.len() == lst1.len() => {
                for i in 0..lst0.len() {
                    if self.set_values_in_context(&lst0[i], &lst1[i], values_defined_here) {
                        return false;
                    }
                }
                ...
                if lst0.len() >= 2 && lst1.len() + 1 > lst0.len() =>
            {
                ...
                for i in 0..lst0.len() {
                    if i == lst0.len() - 1 {
                        ...
                        if !self.set_values_in_context(&lst0[i], &lst1, values_defined_here) {
                            return false;
                        }
                        ...
                        let lst1_val = &lst1[0];
                        lst1 = &lst1[1..];
                        if !self.set_values_in_context(&lst0[i], &lst1_val, values_defined_here) {
                            return false;
                        }
                    }
                }
            ...
            (Syntax::Map(lhs), Syntax::Map(rhs)) => {
                for (key, (val, is_id)) in lhs.iter() {
                    if let Some((rhs, _)) = rhs.get(key) {
                        ...
                        if !self.set_values_in_context(&val, &rhs, values_defined_here) {
                            return false;
                        }
                    } else {
                        return false;
